...
//! without `LD_LIBRARY_PATH`. Publish the same normalized list through this
//! crate's `links = "iris_rs"` metadata so direct downstream crates can relay
//! it from `DEP_IRIS_RS_RPATHS` for their own test/example binaries.
//!
