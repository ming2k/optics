//! Re-emit the rpaths published by `prism-sys` (via its `links` metadata) so
//! that *this* crate's binaries — integration tests and examples — find the
//! prism (and flux, which lives in flux's build dir) shared libraries in the
...
fn main() {
    // Rpaths are a unix concept; on Windows prism-sys stages the DLLs next
    ...
    for var in ["DEP_PRISM_RPATHS", "DEP_FLUX_RPATHS"] {
