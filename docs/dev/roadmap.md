...
| 8.3    | **Done** | GPU memory production hardening (ADR-0020): retire queue generalised to buffers/meshes/targets; all wait-idle calls serialised against submits via `queue_lock` (fixes an ANV driver double-free); `VK_EXT_memory_budget` enabled-when-advertised with a reclaim-first budget gate on pool growth; reclaim-and-retry on OOM with uniform `FLUX_ERROR_OUT_OF_MEMORY`; `VkMemoryDedicatedRequirements` honoured; public `flux_device_memory_stats` with dma-buf accounting and test-side leak gates; 64 MiB-capped staging cache for one-shot uploads/readbacks; `vkAllocateMemory` fault-injection test; debug-utils object naming. |
| 8.4    | **Done** | Upload pipeline + teardown waits (ADR-0021): public `flux_uploads_begin`/`flux_uploads_flush` collapse bulk uploads into one graphics-queue submission (auto-flush at `flux_surface_begin_frame` and at device teardown; recording lock-separated from staging memcpy so loader threads overlap); surface release/resize replace device-wide `vkDeviceWaitIdle` with surface-scoped quiescent waits (per-slot fences offscreen, queue wait windowed); `prefersDedicated` honoured only at ≥ 1 MiB so liberal drivers can't burn the allocation count. New `upload_batch` integration test (pixel-asserted correctness, auto-flush, steady-state accounting). |
| 9      | Partial  | Reference-doc coverage of every exported symbol: **done** — [docs/reference/symbols.md](../reference/symbols.md) covers all 238 exported functions (verified against `nm -D` on the built library). HDR-surface end-to-end test (rendered + presented): pending; needs a real HDR display. |
