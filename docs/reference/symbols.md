...
| `flux_surface_color_space_desc` | Optional `flux_surface_desc.next` extension listing presentable color spaces, most preferred first (ADR-0069). Flux negotiates the first entry the swapchain supports and reports the winner via `flux_surface_info.color_space`. Without it, `hdr_preferred` maps to `[BT2020_PQ, SCRGB, SRGB]`. Offscreen, the first entry with a workable container is adopted (container follows the transfer; see `flux_surface_offscreen_format_desc`). |
| `flux_surface_offscreen_format_desc` | Optional `flux_surface_desc.next` extension constraining an offscreen surface's pixel container, most preferred first. Suitability follows the written space's transfer: sRGB/gamma accept any colour format, PQ/HLG require `FLUX_FORMAT_RGB10A2_UNORM` or `FLUX_FORMAT_RGBA16_SFLOAT`, linear requires `FLUX_FORMAT_RGBA16_SFLOAT`. Ignored on windowed surfaces. |
...
| `flux_shadow_filter_create` | Creates a reusable frame-slot shadow filter whose ping/pong/output images are isolated per frame-in-flight slot — the realtime path for per-frame compositors. |
| `flux_shadow_filter_retain` | Increments the reusable shadow filter refcount. |
| `flux_shadow_filter_release` | Decrements the refcount and frees per-slot images at zero. |
| `flux_shadow_filter_apply` | Records the three shadow passes through the current frame slot: no transient-pool leases, no device-wide wait; parameters may vary every call. |
