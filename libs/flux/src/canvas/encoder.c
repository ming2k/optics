...
typedef enum flux_cmd_kind : uint8_t {
    FLUX_CMD_DRAW_GEOM = 1, FLUX_CMD_DRAW_GLYPH_RUN, FLUX_CMD_SAVE,
    FLUX_CMD_RESTORE, FLUX_CMD_CLIP_RECT, FLUX_CMD_SAVE_LAYER,
    FLUX_CMD_TRANSLATE, FLUX_CMD_SCALE, FLUX_CMD_ROTATE, FLUX_CMD_TRANSFORM,
} flux_cmd_kind;
...
typedef struct { flux_cmd_header header; flux_rect rect; } flux_cmd_clip;
typedef struct { flux_cmd_header header; flux_rect bounds; float opacity; } flux_cmd_save_layer;
typedef struct { flux_cmd_header header; flux_mat3x2 matrix; } flux_cmd_transform;
typedef struct { flux_cmd_header header; float x, y; } flux_cmd_vec2;
typedef struct { flux_cmd_header header; float value; } flux_cmd_scalar;
...
static void encoder_fail(flux_encoder *enc, flux_result error) {
...
static bool encoder_ready(flux_encoder *enc) {
    if (!enc) return false;
    if (enc->finished) encoder_fail(enc, FLUX_ERROR_INVALID_STATE);
    return enc->error == FLUX_OK;
}

static void *encoder_push(flux_encoder *enc, flux_cmd_kind kind, size_t bytes) {
    if (!encoder_ready(enc)) return nullptr;
    if (bytes > UINT32_MAX - 7u || enc->count == UINT32_MAX) {
        encoder_fail(enc, FLUX_ERROR_OUT_OF_RANGE);
        return nullptr;
    }
    ...
    if (required > enc->capacity) {
        ...
        while (cap < required && cap <= enc->budget / 2u) cap *= 2u;
        if (cap < required || cap > enc->budget) cap = enc->budget;
...
flux_result flux_encoder_create(const flux_encoder_desc *desc, flux_encoder **out) {
    ...
    *out = nullptr;
    ...
    if (!enc) return FLUX_ERROR_OUT_OF_MEMORY;
...
static bool finite_rect(flux_rect r) {
    return isfinite(r.x) && isfinite(r.y) && isfinite(r.w) && isfinite(r.h) &&
           r.w >= 0 && r.h >= 0 && isfinite(r.x + r.w) && isfinite(r.y + r.h);
}

static bool valid_geometry(const flux_geometry *g) {
    if (!g || !isfinite(g->stroke_width) || g->stroke_width < 0) return false;
    switch (g->kind) {
    case FLUX_GEOM_RECT: return finite_rect(g->rect.rect);
    ...
    case FLUX_GEOM_CIRCLE:
        return isfinite(g->circle.cx) && isfinite(g->circle.cy) &&
               isfinite(g->circle.radius) && g->circle.radius >= 0;
    case FLUX_GEOM_LINE:
        return isfinite(g->line.x0) && isfinite(g->line.y0) &&
               isfinite(g->line.x1) && isfinite(g->line.y1);
    case FLUX_GEOM_PATH: {
        const flux_path *p = g->path.path;
        if (!p || p->dropped || (p->count && !p->segments)) return false;
        for (uint32_t i = 0; i < p->count; i++) {
            const flux_path_segment *seg = &p->segments[i];
            if (seg->op > FLUX_PATH_CLOSE) return false;
            unsigned n = seg->op == FLUX_PATH_CLOSE ? 0 :
                         seg->op == FLUX_PATH_CUBIC ? 6 : seg->op == FLUX_PATH_QUAD ? 4 : 2;
            for (unsigned j = 0; j < n; j++) if (!isfinite(seg->pts[j])) return false;
        }
        return true;
    }
...
static bool valid_brush(const flux_brush *b) {
    if (!isfinite(b->opacity) || b->opacity < 0 || b->opacity > 1 ||
        b->blend < FLUX_BLEND_SRC_OVER || b->blend > FLUX_BLEND_MULTIPLY) return false;
    if (b->kind == FLUX_BRUSH_SOLID) return true;
    ...
    if (!isfinite(g->start.x) || !isfinite(g->start.y) || !isfinite(g->end.x) ||
        !isfinite(g->end.y) || !isfinite(g->radius) || g->radius < 0 ||
        !g->stops.count || g->stops.count > FLUX_GRADIENT_MAX_STOPS) return false;
    float prev = 0;
    for (uint32_t i = 0; i < g->stops.count; i++) {
        float t = g->stops.stops[i].t;
        if (!isfinite(t) || t < prev || t > 1) return false;
        prev = t;
    }
    return true;
}
...
void flux_encoder_draw_geometry(flux_encoder *enc, const flux_geometry *geom,
    ...
    flux_cmd_draw_geom *cmd = encoder_push(enc, FLUX_CMD_DRAW_GEOM,
        sizeof(*cmd) + (size_t)count * sizeof(flux_path_segment));
    if (!cmd) return;
    cmd->geom = *geom;
    cmd->brush = b;
    ...
    if (geom->kind == FLUX_GEOM_PATH) {
        ...
        if (count) memcpy(cmd + 1, geom->path.path->segments, (size_t)count * sizeof(flux_path_segment));
    }
    if (b.kind == FLUX_BRUSH_IMAGE_PATTERN) {
...
void flux_encoder_draw_glyph_run(flux_encoder *enc, const flux_glyph_run_desc *desc) {
    if (!encoder_ready(enc)) return;
    if (!desc || desc->type != FLUX_TYPE_GLYPH_RUN_DESC ||
        (desc->quad_count && !desc->quads) ||
    ...
    if (ext && (ext->type != FLUX_TYPE_GLYPH_RUN_HOST_ATLAS_DESC || ext->next)) {
        encoder_fail(enc, FLUX_ERROR_UNSUPPORTED);
        return;
    }
    ...
    size_t cov = desc->host_coverage ? (size_t)desc->host_atlas_w * desc->host_atlas_h : 0;
    ...
    for (uint32_t i = 0; i < desc->quad_count; i++) {
        ...
        if (!finite_rect((flux_rect){q->sx, q->sy, q->sw, q->sh})) {
            encoder_fail(enc, FLUX_ERROR_INVALID_ARGUMENT);
            return;
        }
    }
    flux_cmd_glyph_run *cmd = encoder_push(enc, FLUX_CMD_DRAW_GLYPH_RUN,
                                          sizeof(*cmd) + quads + cov);
    if (!cmd) return;
    cmd->desc = *desc;
    ...
    if (quads) memcpy(payload, desc->quads, quads);
    if (cov) memcpy(payload + quads, desc->host_coverage, cov);
...
void flux_encoder_save(flux_encoder *enc) {
    ...
    if (encoder_push(enc, FLUX_CMD_SAVE, sizeof(flux_cmd_header))) enc->depth++;
}

void flux_encoder_restore(flux_encoder *enc) {
    if (!encoder_ready(enc)) return;
    if (!enc->depth) {
        encoder_fail(enc, FLUX_ERROR_INVALID_STATE);
        return;
    }
    if (encoder_push(enc, FLUX_CMD_RESTORE, sizeof(flux_cmd_header))) enc->depth--;
}

void flux_encoder_clip_rect(flux_encoder *enc, flux_rect r) {
    if (!finite_rect(r)) { encoder_fail(enc, FLUX_ERROR_INVALID_ARGUMENT); return; }
...
static void encoder_vec2(flux_encoder *enc, flux_cmd_kind kind, float x, float y) {
    if (!isfinite(x) || !isfinite(y)) { encoder_fail(enc, FLUX_ERROR_INVALID_ARGUMENT); return; }
    flux_cmd_vec2 *cmd = encoder_push(enc, kind, sizeof(*cmd));
    if (cmd) { cmd->x = x; cmd->y = y; }
}
void flux_encoder_translate(flux_encoder *enc, float x, float y) { encoder_vec2(enc, FLUX_CMD_TRANSLATE, x, y); }
void flux_encoder_scale(flux_encoder *enc, float x, float y) { encoder_vec2(enc, FLUX_CMD_SCALE, x, y); }
void flux_encoder_rotate(flux_encoder *enc, float radians) {
    if (!isfinite(radians)) { encoder_fail(enc, FLUX_ERROR_INVALID_ARGUMENT); return; }
    flux_cmd_scalar *cmd = encoder_push(enc, FLUX_CMD_ROTATE, sizeof(*cmd));
    if (cmd) cmd->value = radians;
}
void flux_encoder_transform(flux_encoder *enc, flux_mat3x2 m) {
    ...
    if (!isfinite(a.x) || !isfinite(a.y) || !isfinite(b.x) || !isfinite(b.y)) {
        encoder_fail(enc, FLUX_ERROR_INVALID_ARGUMENT); return;
    }
...
flux_result flux_encoder_finish(flux_encoder *enc, flux_display_list **out) {
    if (!out) return FLUX_ERROR_INVALID_ARGUMENT;
    *out = nullptr;
    if (!enc) return FLUX_ERROR_INVALID_ARGUMENT;
    if (enc->finished) return FLUX_ERROR_INVALID_STATE;
    enc->finished = true;
    if (enc->depth) encoder_fail(enc, FLUX_ERROR_INVALID_STATE);
    if (enc->error != FLUX_OK) return enc->error;
    flux_display_list *list = calloc(1, sizeof(*list));
    if (!list) { encoder_fail(enc, FLUX_ERROR_OUT_OF_MEMORY); return enc->error; }
...
flux_display_list *flux_display_list_retain(flux_display_list *list) {
    if (list) atomic_fetch_add_explicit(&list->refs, 1, memory_order_relaxed);
    return list;
}
void flux_display_list_release(flux_display_list *list) {
    if (!list || atomic_fetch_sub_explicit(&list->refs, 1, memory_order_acq_rel) != 1) return;
    commands_release(list->commands, list->size);
    free(list);
}
size_t flux_display_list_size(const flux_display_list *list) { return list ? list->size : 0; }
uint32_t flux_display_list_command_count(const flux_display_list *list) { return list ? list->count : 0; }
