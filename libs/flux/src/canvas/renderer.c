...
/* Shader bytecode. The primary path is C23 #embed; MSVC cl and Apple
 * Clang lack it, so meson additionally generates <name>_spv.h via
 * tools/spv2h.py with the same bytes, and those builds take the #else
 * branch. FLUX_SHADER_NO_EMBED forces the generated-header path so it
 * stays compile-tested on #embed-capable hosts. Both branches define
 * the same arrays, so the call sites below are identical either way. */
#if defined(__has_embed) && !defined(FLUX_SHADER_NO_EMBED)
#if __has_embed("canvas_solid.vert.spv") && __has_embed("canvas_solid.frag.spv") &&                \
    __has_embed("canvas_gradient.frag.spv") && __has_embed("canvas_image.frag.spv") &&             \
    __has_embed("canvas_sdf.frag.spv") && __has_embed("canvas_glyph.frag.spv") &&                  \
    __has_embed("canvas_output.vert.spv") && __has_embed("canvas_output.frag.spv")
#define FLUX_CANVAS_SHADERS_EMBED 1
#endif
#endif

#ifdef FLUX_CANVAS_SHADERS_EMBED
...
alignas(uint32_t) static const unsigned char canvas_output_vert_spv[] = {
#embed "canvas_output.vert.spv"
};
alignas(uint32_t) static const unsigned char canvas_output_frag_spv[] = {
#embed "canvas_output.frag.spv"
};
#else
#include "canvas_glyph_frag_spv.h"
#include "canvas_gradient_frag_spv.h"
#include "canvas_image_frag_spv.h"
#include "canvas_output_frag_spv.h"
#include "canvas_output_vert_spv.h"
#include "canvas_sdf_frag_spv.h"
#include "canvas_solid_frag_spv.h"
#include "canvas_solid_vert_spv.h" /* generated by tools/spv2h.py; same arrays */
#endif

/* ccache does not hash C23 #embed payloads (observed 4.13.6), so a shader
 * edit could reuse a stale object. The generated manifest folds the .spv
 * bytes' hash into this TU's preprocessed source — and its cache key. */
...
static flux_result build_canvas_pipeline(flux_device *device, VkFormat color_format,
    ...
    const unsigned char *vert_spv = canvas_solid_vert_spv;
    size_t vert_spv_size = sizeof(canvas_solid_vert_spv);
    ...
    case CANVAS_PIPE_SDF:
        ...
        break;
    case CANVAS_PIPE_OUTPUT:
        /* ADR-0069 output transform: fullscreen-triangle vertex stage,
         * no vertex pulling. */
        vert_spv = canvas_output_vert_spv;
        vert_spv_size = sizeof(canvas_output_vert_spv);
        frag_spv = canvas_output_frag_spv;
        frag_spv_size = sizeof(canvas_output_frag_spv);
        break;
    default:
        break; /* SOLID, COVER_SOLID, STENCIL_WRITE use the solid frag */
    }

    VkShaderModule vs = make_module(d, vert_spv, vert_spv_size);
     ...
     * OUTPUT (ADR-0069) blits the resolved intermediate verbatim; blending
     * must stay off or the destination would bleed through the encode. */
    VkPipelineColorBlendAttachmentState ba = {
        .blendEnable = id == CANVAS_PIPE_OUTPUT ? VK_FALSE : VK_TRUE,
