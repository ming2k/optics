 ...
 * where the compiler lacks it — see renderer.c for the full rationale. */
    ...
    __has_embed("effect_liquid_glass.comp.spv") && __has_embed("effect_storage_clear.comp.spv")
...
#include "effect_liquid_glass_spv.h"
#include "effect_storage_clear_spv.h"
#endif
...
flux_result flux_liquid_glass_filter_apply(flux_liquid_glass_filter *filter, flux_frame *frame,
    ...
    *out = nullptr;
    ...
    if (!state)
        return FLUX_ERROR_OUT_OF_MEMORY;
    flux_platform_mutex_lock(&state->lock);
    if (clear_count > 0u)
    ...
    flux_platform_mutex_unlock(&state->lock);
    if (result != FLUX_OK)
        return result;
