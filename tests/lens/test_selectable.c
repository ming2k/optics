...
static void test_default_theme_has_no_decorative_indicator(void) {
    CHECK(lens_theme_default().active_indicator_width == 0.0f);
    CHECK(lens_theme_dark().active_indicator_width == 0.0f);

    lens *ui = NULL;
    CHECK(lens_create(&(lens_desc){0}, &ui) == FLUX_OK);
    lens_theme theme = lens_theme_dark();
    theme.active_indicator_width = -1.0f;
    lens_set_theme(ui, theme);
    CHECK(lens_get_theme(ui).active_indicator_width == 0.0f);
    lens_destroy(ui);
}
...
static void test_opt_in_indicator_has_square_left_edge(void) {
    lens *ui = NULL;
    CHECK(lens_create(&(lens_desc){0}, &ui) == FLUX_OK);
    lens_theme theme = lens_theme_dark();
    ...
    theme.active_indicator_width = 4.0f;
    lens_set_theme(ui, theme);
    ...
    CHECK(p[0] > 250 && p[1] < 5 && p[2] < 5); /* square red rail */
    pixel(fb, stride, 6, 1, p);
    CHECK(p[0] < 5 && p[1] > 250 && p[2] < 5); /* squared selected fill */
    pixel(fb, stride, 98, 1, p);
    CHECK(p[0] < 5 && p[1] < 5 && p[2] < 5); /* rounded right corner */

    flux_canvas_destroy(canvas);
    lens_destroy(ui);
}

static void test_selected_surface_uses_theme_without_indicator(void) {
    lens *ui = NULL;
    CHECK(lens_create(&(lens_desc){0}, &ui) == FLUX_OK);
    lens_theme theme = lens_theme_dark();
    theme.color_active = flux_color_rgba_premul(0, 255, 0, 255);
    theme.color_accent = flux_color_rgba_premul(255, 0, 0, 255);
    theme.corner_radius = 10.0f;
    theme.active_indicator_width = 0.0f;
    lens_set_theme(ui, theme);
...
int main(void) {
    test_default_theme_has_no_decorative_indicator();
    test_selectable_click();
    test_selectable_selected_and_disabled();
    test_opt_in_indicator_has_square_left_edge();
    test_selected_surface_uses_theme_without_indicator();
    return TEST_REPORT();
}
